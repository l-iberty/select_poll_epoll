...
int main() {
  ...
  // 将 listen_sock 与读事件关联, 并注册到 epollfd. 当有客户机发起连接时,
  // 相应的分组达到连接队列 (《Unix网络编程》中有详述), 则 listen_sock
  // 可读而不会阻塞, 从而导致 epoll_wait 返回.
  ev.events = EPOLLIN;
  ...
  for (;;) {
    ...
    for (int i = 0; i < nfds; i++) {
      if (events[i].data.fd == listen_sock) {
        ...
        conn_sock =
            accept(listen_sock, (struct sockaddr *)&cli_addr, &cli_size);
        ...
        // 对于 accept 返回的客户端连接套接字 'conn_sock', 在 epoll
        // 模型中不再使用 recv 函数读取数据, 而是将其注册到 epollfd,
        // 使得在下一轮循环中作为读事件被处理. (debug时发现, conn_sock
        // 和读事件中的 sockfd 等值)
        ...
        if ((sockfd = events[i].data.fd) == -1) continue;
        ...
        // 对客户端连接套接字 conn_sock 的读取完毕, 之后需要向该连接套接字写入,
        // 以此向客户端发送消息. epoll 模型中不使用 send, 而是向 epollfd
        ...
        // 通过debug证实, 对于同一个客户端连接, 之后如果客户端再次发送消息,
        // 在读事件中处理的 file descriptor 与此处的sockfd 等值.
        ev.events = EPOLLIN | EPOLLET;
        ev.data.fd = sockfd;
        if (epoll_ctl(epollfd, EPOLL_CTL_MOD, sockfd, &ev) == -1) {
